  ...
  if (manifest.content_scripts) {
    ...
    if (Array.isArray(output)) {
      if (
        ...
        output.reduce((r, x) => r.add(_nullishCoalesce$1(x.chunkFileNames, () => ( 'no cfn'))), new Set())
          .size > 1
      )
    ...
    const resources = [
      ...
      ...cache.contentScripts.map((x) => slash(path.relative(cache.srcDir, x))),
    ];
    ...
    manifest.web_accessible_resources = _nullishCoalesce$1(manifest.web_accessible_resources, () => ( []));
...
,
  ...
  if (_optionalChain$2([iifeJsonPaths, 'optionalAccess', _ => _.length])) this.warn('`options.iifeJsonPaths` is deprecated');
  ...
  return {
    ...
    options(options) {
      ...
      if (!cache.manifest) {
        ...
        const fullManifest = {
          ...
          version: [...(_nullishCoalesce$2(_optionalChain$3([pkg, 'access', _ => _.version, 'optionalAccess', _2 => _2.matchAll, 'call', _3 => _3(/\d+/g)]), () => ( [])))].join('.'),
    ...
    load(id) {
      if (id === stubChunkNameForCssOnlyCrx) {
        ...
        const code$1 = code.replace('%PATH%', JSON.stringify(target));
        return { code: code$1 }
      }
          ...
          .map((scriptPath) => {
            ...
            const source =
              ...
              wrapperScript.replace(
                ...
                JSON.stringify(slash(path.relative('assets', scriptPath))),
              );
  ...
  return {
    ...
    generateBundle({ plugins = [] }, bundle) {
      ...
      if (executeScript) {
        ...
        _optionalChain$4([manifest, 'access', _ => _.background, 'optionalAccess', _2 => _2.scripts, 'optionalAccess', _3 => _3.unshift, 'call', _4 => _4(executeScriptPolyfillPath)]);
      }
    ...
    generateBundle({ dir }, bundle) {
      ...
      updateManifest(
        (manifest) => {
          ...
          if (manifest.manifest_version === 3) {
            const swPath =
              _nullishCoalesce$3(_optionalChain$5([manifest, 'access', _ => _.background, 'optionalAccess', _2 => _2.service_worker]), () => ( 'service_worker.js'));
            ...
            lodash.set(
              manifest,
              'background.scripts',
              (_nullishCoalesce$3(_optionalChain$5([manifest, 'access', _3 => _3.background, 'optionalAccess', _4 => _4.scripts]), () => ( []))).concat([cache.bgScriptPath]),
            );
          ...
          manifest.content_scripts = _optionalChain$5([ctScripts, 'optionalAccess', _5 => _5.map, 'call', _6 => _6(({ js = [], ...rest }) => ({
